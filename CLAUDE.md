...
**Stack:** React 19, TypeScript 5, Vite 8, Vitest 4, React Router 7, Clerk
**What it does:** Golf pool frontend for the Masters tournament. Coordinators manage pools via a Clerk-authenticated admin area. Entrants submit picks via public entry links or the club entry form. Two clubs (RVCC, Crestmont) share components but have different pick rules.
...
**What's allowed:**
- React ecosystem packages (routing, testing library, Clerk auth).
...
**Env vars:**
- `VITE_CLERK_PUBLISHABLE_KEY` — Clerk public key for coordinator auth
...
4. **Routes are club-scoped.** All user-facing routes are prefixed `/:clubCode/`. The root `/` shows the marketing page. New club-specific pages must follow the `/:clubCode/` pattern.
